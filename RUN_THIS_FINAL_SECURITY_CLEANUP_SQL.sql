...
to authenticated
using (
  lower(coalesce(auth.jwt() ->> 'email','')) = 'demo-admin@copilotdemo.com'
...
to authenticated
using (lower(coalesce(auth.jwt() ->> 'email','')) = 'demo-admin@copilotdemo.com')
with check (lower(coalesce(auth.jwt() ->> 'email','')) = 'demo-admin@copilotdemo.com');
...
using (lower(coalesce(auth.jwt() ->> 'email','')) = 'demo-admin@copilotdemo.com' or exists (select 1 from public.accounts a where a.id = account_id));
create policy payment_plans_insert_final
on public.payment_plans for insert to authenticated
with check (lower(coalesce(auth.jwt() ->> 'email','')) = 'demo-admin@copilotdemo.com' or exists (select 1 from public.accounts a where a.id = account_id));
